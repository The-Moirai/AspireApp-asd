...
class Scheduler:
    ...
    def createState_time(self, task, ncp_list, vms):
        ...
        task_nums = [vm.task_number for vm in vms]
        max_task = max(1, max(task_nums))
        for task_num in task_nums:
            x[index] = task_num / max_task
            index += 1
        return x
    ...
    def taskReward(self, action, vms):
        # 提取所有VM的任务数
        task_nums = [vm.task_number for vm in vms]
        if not task_nums:
            return 0.0  # 无VM时返回中性值

        current_task = task_nums[action]
        min_task = min(task_nums)
        max_task = max(task_nums)
        ...
        # 中间值线性映射到[-1,1]（任务数越大，奖励越低）
        normalized = (current_task - min_task) / (max_task - min_task)  # 归一化到[0,1]
        reward = 1 - 2 * normalized  # 映射到[-1,1]
        return reward
        

    def reward2(self, state, action, task, vms):
        time_r = self.timeReward2(state, action)
        cpu_r = self.cpuReward(state, action)
        task_r = self.taskReward(action, vms)

        if self.debug:
            print(
                "[reward] task.id: {}, time_r: {:.2f}, cpu_r: {:.2f}, load_balance_r: {:.2f}".format(
                    task.id,
                    time_r,
                    cpu_r,
                    task_r
        ...
        # return (time_r + cpu_r) / 4 + task_r / 2
        return (time_r + cpu_r + task_r) / 3
    ...
    def runDQN(self, task, ncp_list, vms, done):
        state = self.createState_time(task, ncp_list, vms)
