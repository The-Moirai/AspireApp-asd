...
class real_node:
    def __init__(self,port):
        self.position=random.randint(1,1200),random.randint(1,800)
        self.ip=get_non_loopback_ip()
        self.radius = 40
        self.neighbors = []
        ...
        self.cpu_memory=500*1024**2#节点的cpu运行内存
        self.cpu_used_memory=100*1024*1024#节点已使用cpu运行内存
    ...
    def refresh_info(self):
        self.time=time.time()
        #print(f"refresh time: {self.time}")
        if(self.port==5002):
            #print("my port is 5002 1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            ...
            try:
                ...
                memory_total = memory_usage.total   # 转换 GB
                memory_used = memory_usage.used   # 转换为 GB
                memory_free = memory_usage.free    # 转换为 GB
            ...
            cpu_used_memory = cpu_memory_info.used    # 转换为 GB
            cpu_free_memory = cpu_memory_info.available  # 转换为 GB
            cpu_memory_percent = cpu_memory_info.percent
            self.cpu_memory=cpu_free_memory
            self.cpu_used_memory=cpu_used_memory
            self.cpu_used_rate=self.cpu_used_memory/(self.cpu_memory+self.cpu_used_memory)*100
            ...
            # self.x=random.randint(0,135)
            # self.y=random.randint(0,85)
            self.x=random.randint(0,1200)
            self.y=random.randint(0,800)
            freq = psutil.cpu_freq()
            self.deal_speed=freq.current
            self.left_bandwidth=random.randint(500000,10000000)

            self.cpu_used_rate=self.cpu_used_memory/(self.cpu_memory) * 100 + random.randint(-5,10)
            #print(f"my port is not 5002 and my cpu_used_rate is{self.cpu_used_rate} my cpu_memory is {self.cpu_memory}")
            # #print(f"已用内存: {cpu_used_memory:.2f} GB")
            # #print(f"剩余内存: {cpu_free_memory:.2f} GB")
            # #print(f"内存使用率: {cpu_memory_percent:.2f}%")
            self.connection=[]
...
def get_network_bandwidth_once(interface, interval=1):
    ...
    sent_mbps = (sent_bytes * 8) / (interval)  # 发送速度 Mbps
    recv_mbps = (recv_bytes * 8) / (interval)  # 接收速度 Mbps
...
def get_my_node(port):
    ...
    try:
        memory_usage = psutil.disk_usage('/')
        memory_total = memory_usage.total    # 转换为 GB
        memory_used = memory_usage.used      # 转换为 GB
        memory_free = memory_usage.free      # 转换为 GB
        memory_percent = memory_usage.percent
    ...
    # 获取内存使用情况
    cpu_memory_info = psutil.virtual_memory()
    cpu_total_memory = cpu_memory_info.total    # 转换为 GB
    cpu_used_memory = cpu_memory_info.used      # 转换为 GB
    cpu_free_memory = cpu_memory_info.available    # 转换为 GB
    cpu_memory_percent = cpu_memory_info.percent
